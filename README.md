...
- **运行 `.exe` 发布版**：无额外依赖，免安装 AutoHotkey / Python，解压即用。程序以普通标准权限（`asInvoker`）运行，双击即可无弹窗启动。
- **运行 `.ahk` 源码版**：需要安装 [AutoHotkey v2.0+](https://www.autohotkey.com/)，双击 `hd2_chat.ahk` 即可直接运行。
...
[Debug]
EnableDebugLog=0  ; 1=启用调试日志, 0=禁用 (默认关闭)
...
1. **单文件直接运行**：从 GitHub Release 下载 `HD2ChatOverlay.exe`，直接双击运行（程序默认以普通用户标准权限 `asInvoker` 启动，无需管理员权限）。
...
1. **权限说明与管理员权限（以管理员身份运行）**：
   - **普通模式（默认）**：程序默认以普通用户权限运行即可与标准权限启动的《绝地潜兵 2》完美协同交互，无需任何 UAC 提权弹窗。
   - **特殊情况**：若您的 Steam 或游戏曾勾选过「以管理员身份运行」（处于 High IL），根据 Windows UIPI 机制，您可能需要将本程序也右键选择「以管理员身份运行」以保持同等特权级别。
